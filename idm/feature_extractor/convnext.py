...
class ConvNeXt(nn.Module):
    ...
    def __init__(
        ...
        # Get padding for no downsampling
        # padding = ('same' if not downsample else 0
        # padding will be same for width and no padding for height
        padding = (0, (kernel_size - 1) // 2) if not downsample else 0
        stride = 4 if downsample else (4, 1)
        stride2 = 2 if downsample else (2, 1)
        stem = nn.Sequential(
            nn.Conv2d(
                input_channels, dims[0], kernel_size=kernel_size, stride=stride, padding=padding
            ),
    ...
    def forward_features(self, x):
        ...
        # global average pooling, (N, C, H, W) -> (N, W, C)
        return self.norm(x.mean(-2).permute(0, 2, 1))

    def forward(self, x):
        if x.dim() == 3:  # We assume batch, height, width
            x = x.unsqueeze(1)
